...
/// Entry points referenced by the x86-64 continuation trampolines
/// generated in `main.rs::compile_continuation_trampolines`. The arm64
/// backend uses the pure-Rust `continuation_trampoline` in
/// `reset_shift.rs` instead and never reaches these functions.
///
/// The generated x86-64 trampolines were written against the v1 reset/
/// shift machinery; the new machinery in `reset_shift.rs` doesn't
/// match those code paths. Until the x86-64 trampolines are rewritten
/// against the new design (see `docs/x86-continuation-port.md`), any
/// code that actually reaches these entry points aborts with a clear
/// error.
#[cfg(any(
...
/// Paired with `invoke_continuation_runtime`. Called when a captured
/// continuation's outermost frame returns — the generated stub in
/// `compile_continuation_trampolines` jumps here after realigning SP.
/// Since the x86-64 backend can't invoke a continuation in the first
/// place, this is unreachable, but the symbol must resolve for the
/// x86-64 build to link.
#[cfg(any(
    feature = "backend-x86-64",
    all(target_arch = "x86_64", not(feature = "backend-arm64"))
))]
pub unsafe extern "C" fn continuation_return_trampoline(_value: usize) -> ! {
    panic!("continuation_return_trampoline reached on x86-64 (continuations unsupported on this backend)");
}
