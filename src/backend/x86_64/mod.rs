//! x86-64 code generation backend for Beagle.
//!
//! This module wraps the `LowLevelX86` implementation to conform
...
///
/// This struct wraps `LowLevelX86` and implements `CodegenBackend`,
...
impl CodegenBackend for X86_64Backend {
    ...
    fn fmov_to_float(&mut self, dest: Self::Register, src: Self::Register) {
        self.inner.fmov_to_float(dest, src);
    }
    ...
    fn push_to_stack_indexed(&mut self, reg: Self::Register, offset: i32) {
        self.inner.push_to_stack_indexed(reg, offset);
    }
    ...
    fn link_register(&self) -> Self::Register {
        // x86-64 doesn't have a link register, use R11 as scratch
        R11
    }

    fn load_return_address(&mut self, dest: Self::Register) {
        // On x86-64, return address is on stack at [RBP + 8]
        // (just above the saved RBP from the prologue)
        // Use inner.instructions directly to emit MovRM with raw byte offset
        // (load_from_heap multiplies by 8, which would give wrong offset)
        self.inner.instructions.push(X86Asm::MovRM {
            dest,
            base: RBP,
            offset: 8, // Raw byte offset, not field index
        });
    }
    ...
    fn get_local_byte_offset(&self, local_index: usize) -> isize {
        // Locals start at [FP-24] (FP-8 is the frame header, FP-16 is the prev pointer)
        -(((local_index + 3) * 8) as isize)
    }

    fn zero_register(&self) -> Self::Register {
        // x86-64 doesn't have a zero register, return RAX as sentinel
        // Callers should handle this specially (e.g., XOR reg, reg to zero)
        RAX
    }
