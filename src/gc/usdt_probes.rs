...
// Many probe functions are intentionally kept for DTrace integration
...
mod probes {
    ...
    // Thread state probes - all include thread_count
    ...
    // Thread registration probes - track when threads register/unregister with GC
    ...
    // Thread spawn probe - fires when OS thread is created (before registration)
    fn thread__spawn(thread_count: u32) {}

    // Stop-the-world probes
