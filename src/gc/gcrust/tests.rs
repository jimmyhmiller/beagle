...
fn semi_space_adversarial_int_payload_in_raw_region_not_relocated() {
    ...
    assert!(
        gc.contains(adversarial as *const u8),
        "addr is in from-space"
    );
    ...
    assert_ne!(
        new_referee, referee as u64,
        "traced slot must follow the move"
    );
...
fn semi_space_traced_slot_relocated_regardless_of_value_negative_control() {
    ...
    assert_eq!(
        s0, s1,
        "both traced slots followed the same object to its new home"
    );
}
...
#[cfg_attr(
    ...
    ignore = "detector armed via GCR_GC_VERIFY in release"
)]
...
fn alloc_site_profile_merges_threads_and_salvages_retired() {
    use crate::gc::gcrust::reflect::{AllocSite, TypeKind, TypeMeta};

    static INFO0: TypeInfo = TypeInfo::for_header(Full::SIZE)
        .with_type_id(0)
        .with_fields(1);
    static INFO1: TypeInfo = TypeInfo::for_header(Full::SIZE)
        .with_type_id(1)
        .with_fields(2);
...
fn alloc_site_profile_real_threads_join_then_profile() {
    use crate::gc::gcrust::reflect::AllocSite;
    use std::sync::Arc;
    ...
    heap.set_alloc_sites(vec![AllocSite {
        function: "worker".into(),
        type_id: 0,
        location: String::new(),
    }]);
...
fn alloc_site_profile_dump_concurrent_with_live_worker_no_hang() {
    ...
    for _ in 0..200 {
        ...
        assert!(
            c <= n_allocs,
            "torn/garbage read: count {c} > total {n_allocs}"
        );
...
fn alloc_site_profile_dump_concurrent_resize_stress_asan() {
    ...
    for _ in 0..200 {
        ...
        assert!(
            total <= n_allocs,
            "torn/garbage read: total {total} > {n_allocs}"
        );
    }
...
fn visit_roots_reports_real_roots_only() {
    ...
    assert!(
        seen.contains(&rooted),
        "visit_roots must report the rooted object"
    );
    assert!(
