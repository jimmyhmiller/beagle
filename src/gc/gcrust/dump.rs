...
use crate::gc::gcrust::field::{read_varlen_bytes, read_varlen_count};
use crate::gc::gcrust::heap::Heap;
use crate::gc::gcrust::reflect::{FieldMeta, FieldTy, ScalarKind, TypeKind};
...
pub unsafe fn dump_heap_json(heap: &Heap) -> String {
    ...
    let id_of: HashMap<usize, usize> = nodes
        .iter()
        .enumerate()
        .map(|(i, n)| (n.obj as usize, i))
        .collect();
...
mod tests {
    ...
    fn retained_pure_cycle_does_not_panic() {
        ...
        assert_eq!(
            r[0], 30,
            "the injected fallback root dominates the whole cycle"
        );
        assert_eq!(r[1], 20);
    }
