...
impl ScalarKind {
    ...
    pub const fn size(self) -> usize {
        match self {
            ...
            ScalarKind::I64 | ScalarKind::U64 | ScalarKind::F64 | ScalarKind::Ptr => 8,
        }
    }
...
fn put_kind(out: &mut Vec<u8>, kind: &TypeKind) {
    match kind {
        ...
        TypeKind::Enum {
            ...
            assert!(
                variants.len() <= u16::MAX as usize,
                "reflect: too many variants"
            );
...
pub fn encode(
    ...
    for offs in interior {
        assert!(
            offs.len() <= u16::MAX as usize,
            "reflect: too many interior ptrs"
        );
            ...
            kind: TypeKind::Struct {
                fields: vec![
                    FieldMeta {
                        name: "x".into(),
                        offset: 0,
                        ty: FieldTy::Scalar(ScalarKind::F64),
                    },
                    FieldMeta {
                        name: "y".into(),
                        offset: 8,
    ...
    fn sample_alloc_sites() -> Vec<AllocSite> {
        vec![
            AllocSite {
                function: "main".into(),
                type_id: 1,
                location: "a.gcr:3:9".into(),
            },
            AllocSite {
                function: "build_list".into(),
                type_id: 2,
                location: "a.gcr:7:5".into(),
            },
