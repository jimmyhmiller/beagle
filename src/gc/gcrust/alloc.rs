...
use crate::gc::gcrust::field::{init_header, read_type_id, read_varlen_count, write_varlen_count};
use crate::gc::gcrust::header::{Compact, ObjHeader};
...
impl BumpAllocator {
    ...
    /// Set the cursor to an absolute byte offset. Used to absorb allocations the
    /// JIT inline fast path performed by bumping a mirrored pointer without
    /// going through `alloc`/`alloc_raw`.
    pub fn set_used(&self, offset: usize) {
    ...
    /// Allocate `size` bytes with `align` alignment, without touching any
    /// `TypeInfo` or writing a header. The caller owns the header format —
    /// used by hosts (e.g. Beagle) whose per-object header is self-describing.
    /// Memory is NOT zeroed (the caller stamps every byte). Returns null if
...
impl AtomicBumpAllocator {
    ...
    /// Allocate `size` bytes with `align` alignment, without touching any
    /// `TypeInfo` or writing a header (caller owns the header format). Memory
    /// is NOT zeroed. Thread-safe via a CAS bump loop. Returns null if the
    /// region is exhausted.
    pub fn alloc_raw(&self, size: usize, align: usize) -> *mut u8 {
        loop {
