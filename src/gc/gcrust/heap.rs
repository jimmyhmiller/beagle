...
use crate::gc::gcrust::reflect::{AllocSite, TypeMeta, ValueMeta};
use crate::gc::gcrust::roots::{AtomicRootSet, RootSource};
use crate::gc::gcrust::scan::scan_object;
use crate::gc::gcrust::type_info::{TypeInfo, VarLenKind};

use crate::gc::gcrust::alloc::{Alloc, AtomicBumpAllocator, HeapWalker};
use crate::gc::gcrust::barrier::SATBQueue;
use crate::gc::gcrust::card_table::CardTable;
use crate::gc::gcrust::semi_space::FORWARDING_BIT;
use crate::gc::gcrust::semi_space::PtrPolicy;
use crate::gc::gcrust::statemap::{StatemapTracer, TraceState};
use crate::gc::gcrust::thread::{SiteCounter, ThreadState};
...
pub struct Heap {
    ...
    alloc_window: Box<crate::gc::gcrust::alloc::AllocWindow>,
...
impl Heap {
    ...
    pub fn new<H: ObjHeader>(space_size: usize, type_table: Vec<TypeInfo>) -> Self {
        let heap = Heap {
            ...
            alloc_window: Box::new(crate::gc::gcrust::alloc::AllocWindow::empty()),
    ...
    pub fn alloc_site_profile_report(&self) -> String {
        ...
        for s in &stats {
            let loc = if s.location.is_empty() {
                "-"
            } else {
                s.location.as_str()
            };
    ...
    pub fn alloc_obj<H: ObjHeader>(&self, info: &TypeInfo, varlen_len: usize) -> *mut u8 {
        unsafe { crate::gc::gcrust::alloc::alloc_obj::<H>(self.from_space(), info, varlen_len) }
    }
    ...
    pub fn alloc_window_ptr(&self) -> *const crate::gc::gcrust::alloc::AllocWindow {
        &*self.alloc_window
    }
    ...
    pub fn alloc_nursery_obj<H: ObjHeader>(&self, info: &TypeInfo, varlen_len: usize) -> *mut u8 {
        match &self.nursery_state {
            Some(ns) => unsafe {
                crate::gc::gcrust::alloc::alloc_obj::<H>(&ns.nursery, info, varlen_len)
            },
            None => unsafe {
                crate::gc::gcrust::alloc::alloc_obj::<H>(self.from_space(), info, varlen_len)
            },
        }
    }
    ...
    pub fn gc_stats_summary(&self) -> String {
        ...
        for kind in [GcKind::Minor, GcKind::Major, GcKind::Concurrent] {
            ...
            let reclaimed: u64 = events
                ...
                .sum();
            let promoted: u64 = events
                .iter()
                .filter(|e| e.kind == kind)
                .map(|e| e.promoted_bytes)
                .sum();
            out.push_str(&format!(
    ...
    pub unsafe fn read_barrier(&self, ptr: *mut u8) -> *mut u8 {
        ...
        unsafe { crate::gc::gcrust::barrier::read_barrier_atomic(ptr, self.type_id_offset) }
    }
