...
## beagle.text  (29 fns)
`code-at`, `ws-code?`, `digit-code?`, `repeat-string`, `pad-left`, `pad-right`, `clip-fill`, `capitalize`, `title-case`, `char-set-contains?`, `trim-chars`, `trim-chars-left`, `trim-chars-right`, `replace-all`, `split-with-limit`, `starts-with-any?`, `ends-with-any?`, `contains-any?`, `parse-float`, `hex-digit`, `int->hex`, `fixed`, `round-to`, `commas`, `count-occurrences`, `reverse-string`, `string->bytes`, `bytes->string`, `ellipsize`
